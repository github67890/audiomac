...
function LandingPage() {
  ...
  const [genretest, setgenretest] = React.useState("all");
  const [checkverified, setcheckverified] = React.useState("no");
  ...
  const handlegenre = (e)  => {
   //// "'"+text+"'"
  ;
    setgenretest( e.target.value.trim());
   //// alert(genretest);
  }

  const handlerverified = (e)  => {
  if(checkverified=="yes"){

    setcheckverified("no")
  }
  else{
    setcheckverified("yes")
  }
...
///  console.log(json);
  ...
  var parts = window.location.pathname.split('/');
      var lastSegment = parts.pop() || parts.pop();  // handle potential trailing slash
  var myModule = require('views/database');
const response= await fetch(myModule.servername+"/api/users/getprofilesearch ", {
  method: "post",
  headers: {
    "content-type": "application/x-www-form-urlencoded; charset=utf-8",
  },
...
<Row>
      ...
      <FormLabel component="legend">Searches</FormLabel>
      <RadioGroup aria-label="gender" name="gender1" value={value} onChange={handlechanges}>
      <FormControlLabel value="all" control={<Radio />} label="All Searches" />
      </RadioGroup>  
      ...
      <FormGroup check>
                <Label check>
                  <Input type="checkbox" onChange={handlerverified}></Input>
...
<Col lg="1" md="1" sm="4" xm="4">
 ...
 {profiledata.filter(book => book.verified === checkverified).map((s,i)=> ( <> 
 ...
 <p class="pstyl">{s.totalfollower}</p>
      ...
      {profiledata[i].checksubscribe=='yes'? <> <Button   className=" btn-round submarg" 
                size="sm"
                href=''>
                   Subscribed
                  </Button></>: <>
...
{/* ////////////////////////////////////////////////////////////////////// */}
{alldata.filter(book => book.genre ===genretest).map((s,i)=> ( <> 
...
<>
{albumalldata[i].checksubscribe=='yes'? <> <span className="subedstyl">
                   Subscribed
                   </span></>: <>
                  <Button  onClick={  () => subscribeme(s.uploderid)} className=" btn-round substyl" 
                size="sm"
